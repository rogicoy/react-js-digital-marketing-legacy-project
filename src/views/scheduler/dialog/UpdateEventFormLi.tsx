...
import { GetLiPostReponse } from 'store/scheduler/models';
import { IUpdateEventFormLi } from '../interface';
...
const UpdateEventFormLi: FC<IUpdateEventFormLi> = ({ range, social, postId, onCancel, onUpdate, onDelete }) => {
  // style
  ...
  // graphql
  const {
    query: { linkedinSchedulePostById },
    mutation: { linkedinSchedulePost, linkedinDeleteSchedulePost }
  } = gql;

  const [liUpdatePost, { loading: updateLoading }] = useMutation<{
  ...
  const [liDeletePost, { loading: deleteLoading }] = useMutation<{
    linkedinDeleteSchedulePost: {
      info: string;
      status: number;
      error_message: string;
      data: JSON;
  ...
  const { data: liPostDetails, loading: postLoading } = useQuery<{
    linkedinSchedulePostById: GetLiPostReponse;
  ...
  const postDetails = liPostDetails?.linkedinSchedulePostById;
  ...
  const onDeletePost = async (confirm: boolean) => {
    if (confirm) {
      try {
        const { data } = await liDeletePost({
          variables: {
            id: postId
          }
        });

        if (data && data.linkedinDeleteSchedulePost.status === 200) onDelete(postId);
      } catch (error) {
        console.log(error);
      }
    }
            ...
            <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
              ...
              <DialogContent sx={{ p: 3 }}>
                <Grid container spacing={gridSpacing}>
                  ...
                  <Grid item md={5}>
                    <Grid container md={5} spacing={gridSpacing}>
                      <Grid item xs={12}>
...
export default UpdateEventFormLi;
