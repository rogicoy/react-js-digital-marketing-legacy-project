...
import { IAddEventFormTw } from '../interface';
...
const AddEventFormTw: FC<IAddEventFormTw> = ({ range, social, handleCreate, onCancel, preFill }) => {
  // style
  ...
  // graphql
  const {
    mutation: { twitterSchedulePost }
  } = gql;

  const [twSchedulePost, { loading: postLoading }] = useMutation<{
    twitterSchedulePost: {
      info: string;
      status: number;
  ...
  const onSubmit = async (values: EventTwFormData, { setSubmitting, setErrors }: FormikHelpers<EventTwFormData>) => {
    try {
      ...
      const { data } = await twSchedulePost({
        variables: {
          id: social?.socialId,
          input
        }
      ...
      if (data && data.twitterSchedulePost.status === 200) {
        const objValues = {
          ...values,
          id: data.twitterSchedulePost.info,
          end: values.start,
          title: '',
          color: '#d2e7fa',
          textColor: '#218ae5',
