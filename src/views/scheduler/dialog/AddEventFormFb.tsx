...
import { IAddEventFormFb } from '../interface';
...
const AddEventFormFb: FC<IAddEventFormFb> = ({ range, social, handleCreate, onCancel, preFill }) => {
  // style
  ...
  // graphql
  const {
    mutation: { facebookSchedulePost }
  } = gql;

  const [fbSchedulePost, { loading: postLoading }] = useMutation<{
  ...
  const onSubmit = async (values: EventFbFormData, { setSubmitting, setErrors }: FormikHelpers<EventFbFormData>) => {
    try {
      ...
      const { data } = await fbSchedulePost({
        variables: {
          id: social?.socialId,
          input
        }
      ...
      if (data && data.facebookSchedulePost.status === 200) {
        const objValues = {
          ...values,
          id: data.facebookSchedulePost.info,
          end: values.start,
          title: '',
          color: '#d0e0f2',
          textColor: '#1b69c1',
  ...
  return (
    ...
    >
      {(formikProps) => {
        const { errors, handleChange, handleSubmit, handleBlur, setFieldValue, isSubmitting, touched, values } = formikProps;
