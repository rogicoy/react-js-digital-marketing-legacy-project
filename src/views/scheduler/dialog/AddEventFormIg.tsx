...
import { IAddEventFormIg } from '../interface';
...
const AddEventFormIg: FC<IAddEventFormIg> = ({ range, social, handleCreate, onCancel, preFill }) => {
  // style
  ...
  // graphql
  const {
    mutation: { instagramSchedulePost }
  } = gql;

  const [igSchedulePost, { loading: postLoading }] = useMutation<{
   ...
   * @param param1
   */
  const onSubmit = async (values: EventIgFormData, { setSubmitting, setErrors, setStatus }: FormikHelpers<EventIgFormData>) => {
    try {
      ...
      const { data } = await igSchedulePost({
        variables: {
          id: social?.socialId,
          input
        }
      ...
      if (data && data.instagramSchedulePost.status === 200) {
        const objValues = {
          ...values,
          id: data.instagramSchedulePost.info,
          end: values.start,
          title: '',
          color: '#f9d6e2',
          textColor: '#e23670',
  ...
  return (
    <Formik
      ...
      validationSchema={Yup.object().shape({
        ...
        start: Yup.date(),
        image: Yup.string().required('Please choose your image in the content manager')
      })}
      onSubmit={onSubmit}
    >
      {(formikProps) => {
            ...
            <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
              ...
              <DialogContent sx={{ p: 3 }}>
                <Grid container spacing={gridSpacing}>
                  <Grid item md={4}>
                    <Grid container item spacing={gridSpacing} sx={{ height: '100%' }}>
