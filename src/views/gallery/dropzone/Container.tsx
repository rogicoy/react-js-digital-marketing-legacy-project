...
const Container = (props: DropzoneProps) => {
  ...
  const onDropAccepted = (files: File[]) => {
    ...
    files.forEach((file: File, index: number) => {
      ...
      if (videoFileTypes.includes(file.type)) {
        loadVideo(url).then((dimension: any) => {
          doAddFiles([
            {
              url,
              index,
        ...
        loadImage(url).then((dimension: any) => {
          doAddFiles([
            {
              url,
              index,
  ...
  const handleUpload = () => {
    ...
    validFiles.forEach((file) => {
      doUploadFile(file.ref, acceptedFiles[file.index]);
    });
  };
  ...
  return (
    <Grid container spacing={2}>
      ...
      <Grid item xs={12} container>
              ...
              file.uploadStatus !== 'success' && (
                <Box sx={{ m: 2, position: 'relative' }}>
                  <Grid item xs={5} md={2} lg={1}>
                    {videoFileTypes.includes(file.mimeType || '') && (
                      <CardMedia component="video" sx={{ height: 100, width: 100 }} controls src={file.url} />
                    )}
                    {imageFileTypes.includes(file.mimeType || '') && (
                      <CardMedia component="img" sx={{ height: 100, width: 100 }} src={file.url} alt={file.name} />
                    )}
                  </Grid>
                  {file.uploadStatus === 'ongoing' && (
                    <CircularProgress
                      size={24}
                      sx={{
                        position: 'absolute',
        ...
        {validFiles.length > successfulUploadCount && (
          <Grid item xs={12} container justifyContent="flex-end">
            <Button disableElevation variant="contained" sx={{ width: 160 }} onClick={handleUpload} disabled={ongoingUploadCount > 0}>
              Save
            </Button>
          </Grid>
        )}
