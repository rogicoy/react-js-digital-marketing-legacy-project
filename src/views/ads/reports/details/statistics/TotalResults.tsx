...
// assets
import PersonIcon from 'assets/images/icons/person-in-circle.svg';
...
const useStyles = makeStyles((theme: Theme) => ({
  card: {
    background: 'linear-gradient(180deg, #FFC352 0%, #E88699 100%)',
    color: '#fff',
    overflow: 'hidden',
    position: 'relative',
    '&:after': {
...
const TotalResults: FC<TotalResultsProps> = ({ performance, objective, isLoading }) => {
      ...
      {isLoading ? (
        ...
        <MainCard border={false} className={classes.card} contentClass={classes.content}>
          <Box className={classes.bubbleBox}>
            <img src={PersonIcon} alt="Person icon" className={classes.bubbleImage} />
          </Box>
          ...
          <Grid container direction="column" sx={{ mt: gridSpacing }}>
              ...
              <Grid container alignItems="center">
                <Grid item>
                  <Typography className={classes.cardHeading}>
                    <NumberFormat value={performance.value} displayType="text" decimalScale={2} thousandSeparator />
                  </Typography>
                </Grid>
              </Grid>
            </Grid>
            ...
            <Grid item sx={{ mb: 1.25, width: '100%' }}>
              <Box display="flex">
                <Box flexGrow={1}>
                  <Typography className={classes.subHeading}>{`Total ${helpers.textLowerCase(objective)}`}</Typography>
                </Box>
