...
const PerformingPosts: FC<IPerformingPosts> = ({ mainTitle, link = '/insights/instagram/posts' }) => {
  ...
  const [getInstagramTopPostsLazy, { data: instagramTopPostsData, loading: instagramTopPostsLoading }] = useLazyQuery(
    gql.query.instagramTopPosts,
    {
      fetchPolicy: 'network-only'
    }
  );
  const instagramTopPosts = instagramTopPostsData?.instagramTopPosts;

  const topPostsData: ILikePosts[] = useMemo(
    () =>
      instagramTopPosts?.data?.map(
        (e: any): ILikePosts => ({
          image: e.url,
